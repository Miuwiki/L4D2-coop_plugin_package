...
Action Timer_DisplayHUDInfo(Handle timer)
{
	...
	if( g_show_list & MAXPLAYER_LIST )
	{
		...
		show_hud.slot = 1;
		show_hud.flag = HUD_FLAG_ALIGN_LEFT|HUD_FLAG_NOBG;
...
void DisplayPlayerList()
{
	...
	HUD hud_playerlist;
	static char temp[4],num[2][6],info[2][62];
	int slot;
	for(int i = 0; i < total; i++)
	{
		temp = IsPlayerAlive(survivor[i]) ? "★" : "☆";
		if( i < 9 )
			FormatEx(num[0],sizeof(num[])," %d ",i+1);
		else
			FormatEx(num[0],sizeof(num[]),"%d",i+1);

		if( g_player_killspecial[survivor[i]] < 10 )
				FormatEx(num[1],sizeof(num[]),"   %d   ",g_player_killspecial[survivor[i]]);
		else if( 10 <= g_player_killspecial[survivor[i]] < 100 )
			FormatEx(num[1],sizeof(num[]),"  %d  ",g_player_killspecial[survivor[i]]);
		else if( 100 < g_player_killspecial[survivor[i]] )
			FormatEx(num[1],sizeof(num[])," %d ",g_player_killspecial[survivor[i]]);
 
		if( i & 1 ) // otd.
			FormatEx(info[1],sizeof(info[]),"%s%s|%s ->%N",temp,num[0],num[1],survivor[i]);
		else // even, include 0
			FormatEx(info[0],sizeof(info[]),"%s%s|%s ->%N",temp,num[0],num[1],survivor[i]);

		// i + 1 = total means this is the last one, go to show.
		// i % 2 and i != 0 means that we have two info now, so we go to show and clear.
		if( i + 1 == total || (i > 0 && i & 1) ) 
			FormatEx(hud_playerlist.info,sizeof(hud_playerlist.info),"%s\n%s",info[0],info[1]);
		else
			continue;

		info[0] = "";
		info[1] = "";
		// FormatEx(hud_playerlist.info,sizeof(hud_playerlist.info),"★%s %d: %d/%d→%N\n★%s %d: %d/%d→%N",
		// 		temp[0],
		// 		i+1,
		...
		// 		temp[1],
		// 		i+2,
		// 		g_player_killspecial[survivor[i+1]],
		// 		g_player_headshot[survivor[i+1]],survivor[i+1]);
		hud_playerlist.slot = slot+2;
		hud_playerlist.flag = HUD_FLAG_ALIGN_LEFT|HUD_FLAG_NOBG;
		hud_playerlist.pos  = g_HUDpos[slot+2];
	...
	while( slot < 8 ) // remove the extra slot if player count is reduced.
	{
		RemoveHUD(slot + 2);
		slot++;
	}
